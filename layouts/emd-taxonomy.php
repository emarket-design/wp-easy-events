...
$container = apply_filters('emd_change_container','emd-wrapper','wp_easy_events', 'taxonomy');  
$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'wp_easy_events', 'taxonomy');
...
<?php
	do_action('emd_tax_before_header', 'wp-easy-events',$post->post_type,$queried_object->taxonomy);
	emd_get_template_part('wp-easy-events', 'taxonomy', str_replace("_","-",$queried_object->taxonomy . '-' . $post->post_type) . '-theader');
	do_action('emd_tax_after_header', 'wp-easy-events',$post->post_type,$queried_object->taxonomy);
?>
	</div>
...
<?php
	while ( have_posts() ) : the_post(); ?>
			...
			<?php emd_get_template_part('wp-easy-events', 'taxonomy', str_replace("_","-",$queried_object->taxonomy . '-' . $post->post_type)); ?>
			</div>
...
<?php
	do_action('emd_tax_before_footer', 'wp-easy-events',$post->post_type,$queried_object->taxonomy);
	emd_get_template_part('wp-easy-events', 'taxonomy', str_replace("_","-",$queried_object->taxonomy . '-' . $post->post_type) . '-tfooter');
	do_action('emd_tax_after_footer', 'wp-easy-events',$post->post_type,$queried_object->taxonomy);
?>
</div>
<?php	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'wp_easy_events', 'taxonomy');
	if($has_navigation){	
		global $wp_query;
