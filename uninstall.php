...
if (!empty($tools['remove_data'])) {
	...
	foreach ($app_post_types as $post_type) {
		...
		if (!empty($postslist)) {
			$entity_fields = get_option('wp_easy_events_attr_list');
	...
	$pages_list = get_option('wp_easy_events_setup_pages_list', Array());
	if (!empty($pages_list)) {
...
if (!empty($tools['remove_settings'])) {
	...
	//delete all settings
	$wpdb->query("DELETE FROM $wpdb->options WHERE option_name LIKE 'wp_easy_events\_%';");
}
