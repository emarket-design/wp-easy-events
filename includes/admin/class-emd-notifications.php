...
class Emd_Notifications {
	...
	public function __construct($app) {
		add_action('admin_init', array($this,'emd_notify_export'));
		add_action('wp_ajax_emd_notify_import_file',array($this,'emd_notify_import_file'));
	...
	public static function display_settings_notify($app, $notify_init_list) {
		if (!empty($notify_init_list)) {
			global $title;
			$export_url = admin_url('admin.php?page=' . $app . '_notify&action=export&app=' . $app . '&nonce=' . wp_create_nonce('emd_notify_export'));
			echo '<div class="wrap"><h2>' . $title;
			$shc_list = get_option($app . '_shc_list',Array());
			if(empty($shc_list['has_lite'])){
				echo '<input type="file" id="emd_notify_import_file" data-app="' . $app . '" hidden>';
				echo '<a href="#" class="add-new-h2 emd-import" style="padding:6px 10px;">' . esc_html('Import', 'emd-plugins') . '</a>';
				echo '<a href="' . $export_url . '" class="add-new-h2" style="padding:6px 10px;">' . esc_html('Export', 'emd-plugins') . '</a>';
			}
			else {
	...
	public function emd_notify_export() {
		if(!empty($_GET['action']) && $_GET['action'] == 'export' && !empty($_GET['app']) && $_GET['page'] == $_GET['app'] . '_notify'){
			if(!check_admin_referer('emd_notify_export','nonce')){
				exit;
			}
			$app = sanitize_text_field($_GET['app']);
			$date = date_i18n("d_m_Y");
			$fileName = sanitize_file_name($app . '_notifications_' . $date . '.json');
			$notify_list = get_option($app . "_notify_list",Array());
			$json = json_encode($notify_list);
	...
	public function emd_notify_import_file(){
		check_ajax_referer('emd_notify_import', 'nonce');
		if (!function_exists('wp_handle_upload')) {
			require_once(EMD_ADMIN_DIR . '/includes/file.php');
		}
		if(!empty($_FILES['file'])){
			...
			if (isset($upload['error'])) {
				echo '<p><b>' . __('Error:', 'emd-plugins') . '</b></p>' . $upload['error'];
				die();
			}
			if (!empty($upload['file'])) {
				...
				if(!empty($decoded)){
					update_option($app .'_notify_list',$decoded);
				}
			}
		}
		echo true;
