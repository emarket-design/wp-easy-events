...
/**
 * Emd_Event_Attendee Class
 * @since WPAS 4.0
 */
class Emd_Event_Attendee extends Emd_Entity {
	protected $post_type = 'emd_event_attendee';
	...
	public function __construct() {
		...
		if ($is_adv_filt_ext === 0) {
			add_action('manage_emd_event_attendee_posts_custom_column', array(
				$this,
				'custom_columns'
			) , 10, 2);
			add_filter('manage_emd_event_attendee_posts_columns', array(
				$this,
				'column_headers'
			));
		}
	...
	public static function register() {
		$labels = array(
			'name' => __('Attendees', 'wp-easy-events') ,
			'singular_name' => __('Attendee', 'wp-easy-events') ,
			'add_new' => __('Add New', 'wp-easy-events') ,
			'add_new_item' => __('Add New Attendee', 'wp-easy-events') ,
			'edit_item' => __('Edit Attendee', 'wp-easy-events') ,
			'new_item' => __('New Attendee', 'wp-easy-events') ,
			'all_items' => __('All Attendees', 'wp-easy-events') ,
			'view_item' => __('View Attendee', 'wp-easy-events') ,
			'search_items' => __('Search Attendees', 'wp-easy-events') ,
			'not_found' => __('No Attendees Found', 'wp-easy-events') ,
			'not_found_in_trash' => __('No Attendees Found In Trash', 'wp-easy-events') ,
			'menu_name' => __('Attendees', 'wp-easy-events') ,
		);
		$ent_map_list = get_option('wp_easy_events_ent_map_list', Array());
		$myrole = emd_get_curr_usr_role('wp_easy_events');
		if (!empty($ent_map_list['emd_event_attendee']['rewrite'])) {
			$rewrite = $ent_map_list['emd_event_attendee']['rewrite'];
		} else {
			$rewrite = 'attendee';
		}
		$supports = Array();
		register_post_type('emd_event_attendee', array(
			'labels' => $labels,
			'public' => true,
			'publicly_queryable' => true,
			'show_ui' => true,
			'description' => __('A person or organization attending the event.', 'wp-easy-events') ,
			'show_in_menu' => '',
			'menu_position' => null,
			'has_archive' => false,
			...
			'map_meta_cap' => 'true',
			'taxonomies' => array() ,
			'capability_type' => 'emd_event_attendee',
			'supports' => false,
		));
	}
	/**
	...
	public function set_filters() {
		...
		$search_args = Array();
		$filter_args = Array();
		$this->sing_label = __('Attendee', 'wp-easy-events');
		$this->plural_label = __('Attendees', 'wp-easy-events');
		$this->menu_entity = 'emd_wpe_event';
		$this->boxes['emd_event_attendee_info_emd_event_attendee_0'] = array(
			'id' => 'emd_event_attendee_info_emd_event_attendee_0',
			'title' => __('Attendee Info', 'wp-easy-events') ,
	...
	public function change_content($content) {
		...
		if (get_post_type() == $this->post_type && is_single()) {
			...
			emd_get_template_part($this->textdomain, 'single', 'emd-event-attendee');
	...
	public function add_menu_link() {
		add_submenu_page(null, __('CSV Import/Export', 'wp-easy-events') , __('CSV Import/Export', 'wp-easy-events') , 'manage_operations_emd_event_attendees', 'operations_emd_event_attendee', array(
			$this,
			'get_operations'
		));
	}
	...
	public function get_operations() {
		if (current_user_can('manage_operations_emd_event_attendees')) {
	...
	public function add_top_menu_link() {
		add_submenu_page('edit.php?post_type=emd_wpe_event', __('Attendees', 'wp-easy-events') , __('All Attendees', 'wp-easy-events') , 'edit_emd_event_attendees', 'edit.php?post_type=emd_event_attendee', false);
		add_submenu_page('edit.php?post_type=emd_wpe_event', __('Attendees', 'wp-easy-events') , __('Add New Attendee', 'wp-easy-events') , 'edit_emd_event_attendees', 'post-new.php?post_type=emd_event_attendee', false);
		$myapp = str_replace("-", "_", $this->textdomain);
